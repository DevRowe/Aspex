...
**The buildable-now free-form intent and adapter track - ✅ CHUNKED + SHIPPED (cards 46-58).** Grilled via grill-with-docs (6 decisions), chunked, implemented, and documented: see `46-phase-3-index.md` (index + canonical contracts + the free-form safety model), cards `47`-`58`, `../free-form-intent.md`, and the Phase 3 sections of `../threat-model.md`. Decisions are recorded in ADR-0018-0022 and the *Free-form intent (Phase 3)* glossary section in `../../CONTEXT.md`. Two tracks shipped:

- **Track A — Free-form intent** (cards 47–53) shipped: Ollama + a per-request **JSON-Schema structured-output** constraint turning natural language into **one** structured action, as a **fallback** behind the Phase 1 closed grammar (only `unknown_command` falls through — ADR-0018), reached as a pluggable, mock-first **[[Intent service]]** (ADR-0019), **bounded by construction** — enum output, single-shot, never `confirm`/`dictation_body`/`post`, elevate-confirm (ADR-0020). Serves **voice + a typed [[Intent bar]]**, usable with voice off.
- **Track B — Agent adapters** (cards 54–57) shipped: **codex** (`notify`→`hook-relay`), **opencode** (`opencode serve` `/event` SSE), **cursor** (opt-in signed inbound webhook — a bounded exception to no-public-ingress, ADR-0022). All **observe + deep-link only**, owning agent-local attention; PR-lifecycle stays the github adapter's (ADR-0021).
