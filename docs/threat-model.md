...
The Hub binds `127.0.0.1` and is intended for same-machine access. Under the
north-star realignment it is also reachable from glasses over a private tailnet,
so "on the box" no longer implies "is the user." There is still no automatic
public ingress.
...
Because the Hub is reachable over a tailnet rather than pure loopback, every HTTP
and SSE endpoint requires a locally generated bearer token (ADR-0023). Without
it, any tailnet peer could read the world-model, dispatch actions, and inject
Signals.

The token is generated on first boot and stored in `~/.aspex/config.json`, or
...
is accepted for a local/tailnet stream. The token is compared in constant time
over fixed-length digests, and a missing or wrong token returns `401`.

CORS is unchanged: the token check runs after the CORS middleware, so preflight
`OPTIONS` still succeeds. The `POST /webhooks/cursor` route is the one bearer
exemption, because it is reached by Cursor's cloud and authenticates with its own
HMAC signature instead (ADR-0022). The bundled `aspex hook-relay` and
`aspex preview list` present the token so same-box ingestion still works.
